FUNCTION Get-InterfaceDetails {
    ...
    PROCESS{
        ...
        $interfaces = Invoke-Command -ComputerName $Computer -ScriptBlock {Get-NetAdapter} -ErrorAction SilentlyContinue; # get network adapters 
        $output = @();
           
            foreach ($interface in $interfaces) {#loop through the interfaces and build the custom output
                
                $ipDetails = Invoke-Command -ComputerName $Computer -ScriptBlock {Get-WmiObject Win32_NetworkAdapterConfiguration | Where-Object {$_.interfaceIndex -eq $interface.ifIndex}} | Select-Object * -ErrorAction SilentlyContinue;
